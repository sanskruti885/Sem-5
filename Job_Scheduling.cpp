...
void display_avg(string algo_name, vector<Process> p){
    ...
    cout<<"Process\t\tAvg WT\t\tAvg TAT"<<endl;
    cout<<algo_name<<"\t\t"<<avg_wt<<"\t\t"<<avg_tat<<endl;
}
...
void FCFS(vector<Process> p, int n){
    ...
    p[0].turn_arround = p[0].burst_time;
...
void PriorityScheduling(vector<Process> p, int n){
    sort(p.begin(), p.end(), compare_priority); // Sorting by priority
